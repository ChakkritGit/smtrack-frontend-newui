...
const ManageFirmware = () => {
  ...
  useEffect(() => {
    ...
    if (selectedDevicesOption?.toLowerCase().endsWith(".bin")) {
      const match = selectedDevicesOption.match(/v?(\d+\.\d+\.\d+)/i);
      newFilter = match ? match[1] : "";
    }
  ...
  }, [filterUpdated, selectedDevicesOption, deviceList]);
  ...
  const uploadModalComponent = useMemo(
    () => (
      <dialog ref={uploadModalRef} className="modal overflow-y-scroll py-10">
        <div className="modal-box md:w-160 max-w-3xl h-max max-h-max">
  ...
  const selectUploadModalComponent = useMemo(
    () => (
      ...
      >
        <div className="modal-box min-h-120 w-11/12 max-w-6xl h-max max-h-max">
          <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center justify-end mt-3">
            <Select
              ...
              autoFocus={false}
              className="react-select-container custom-device-select z-75 min-w-full md:min-w-78.75"
          ...
          <div>
            {selectedDevicesOption !== t("selectOTA") && (
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 mt-3 max-h-[calc(100dvh-300px)] overflow-y-scroll">
                  {deviceFwFiltered
                    .filter((items) =>
                      items.id
                        .substring(0, 1)
