...
const HomeCount = (props: DeviceCountPropType) => {
  ...
  const { deviceCount, countFilter } = props
  ...
  const card = [
    ...
    {
      name: t('countConnect'),
      count: deviceCount?.internet,
      time: t('countNormalUnit'),
      icon: <RiSignalWifi1Line />
    },
    {
      name: t('countPlug'),
    ...
    {
      name: t('countSdCard'),
      count: deviceCount?.sdcard,
      time: t('countNormalUnit'),
      icon: <RiSdCardMiniLine />
    },
    {
      name: t('countRepair'),
      count: deviceCount?.repairs,
      time: t('countDeviceUnit'),
      icon: <RiFolderSettingsLine />
    },
    {
      name: t('countWarranty'),
      count: deviceCount?.warranties,
      time: t('countDeviceUnit'),
  ...
  return (
    <div className='flex items-center justify-center flex-wrap gap-4 mt-4 p-4'>
      {card.map((card, index) => (
        <div
          className={`card ${
            countFilter === card.name
              ? 'bg-primary text-base-100'
              : 'bg-base-100'
          } w-36.25 h-31.25 overflow-hidden shadow-xl ${
            card.name === t('countRepair') || card.name === t('countWarranty')
              ? 'hover:scale-105 transition-all duration-300 ease-linear cursor-pointer'
              : ''
          }`}
          key={index}
          onClick={() => changFilter(card.name)}
        >
