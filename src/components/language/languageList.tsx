...
const LanguageList = () => {
  ...
  return (
    <div className="dropdown dropdown-end">
      <div tabIndex={0} role="button" className="flex btn btn-ghost tooltip tooltip-left md:tooltip-bottom" aria-label="Language" data-tip={t('tabLanguage')}>
      ...
      <div tabIndex={0}
        className="dropdown-content bg-base-100 text-base-content rounded-box z-10 top-px mt-16 max-h-[calc(100vh-10rem)] w-40 overflow-y-auto border border-white/5 shadow-2xl outline-1 outline-black/5">
        <ul className="menu menu-sm gap-1 w-full">
          <li onClick={() => changeLanguage('th')}>
            <button className={`${langs === "th" ? "menu-active" : ""} flex items-center justify-start h-9`}>
              <span
          ...
          <li onClick={() => changeLanguage('en')} >
            <button className={`${langs === "en" ? "menu-active" : ""} flex items-center justify-start h-9`}>
              <span
                className="badge badge-sm badge-outline !pl-1.5 !pr-1 pt-px font-mono !text-[.6rem] font-bold tracking-widest opacity-50">
                EN</span>
